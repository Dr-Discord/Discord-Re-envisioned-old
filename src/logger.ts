...
function getTime():string {
  ...
  return `${newHrs}:${newMin}${hrs >= 12 ? "pm" : "am"}`
}

function generateStr(prop:"log"|"warn"|"error") {
  const showTime = internal.get("showTime") ?? false
  let color = "#F52590"
  switch (prop) {
    case "error": 
      color = "#ed4245"
  ...
  return [
    `${prop === "log" ? "%c " : ""}%cDR%c${i18n.name}${showTime ? `%c${getTime()}` : ""}%c\n`, 
    ...
    showTime ? `background: ${color}; margin-left: 5px; margin-bottom: 9px; padding: 2px; border-radius: 4px; color: ${isDark ? "#202124" : "#fff"}` : null,
    "padding-left: 3px"
  ].filter(e => e)
}

export default {
  error: (...input:any[]) => _console.error(
    ...generateStr("error"), ...input
  ),
  warn: (...input:any[]) => _console.warn(
    ...generateStr("warn"), ...input
  ),
  log: (...input:any[]) => _console.log(
    ...generateStr("log"), ...input
  )
}